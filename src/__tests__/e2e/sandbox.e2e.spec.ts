...
test.describe('App Initialization', () => {
  test('shows loading screen then transitions to main UI', async ({ page }) => {
    ...
    await expect(page.getByRole('listbox', { name: 'File list' })).toBeVisible();
  ...
  test('default files are visible in the file explorer', async ({ page }) => {
    ...
    await expect(fileList.getByRole('option', { name: /index\.ts/ })).toBeVisible();
    await expect(fileList.getByRole('option', { name: /utils\.ts/ })).toBeVisible();
  });
...
test.describe('Code Execution — Happy Path', () => {
  ...
  test('running the default code produces output in the terminal', async ({ page }) => {
    await clickRun(page);
  ...
  test('status indicator cycles: Ready → Bundling → Executing → Done', async ({ page }) => {
    const status = page.getByRole('status', { name: 'Execution status' });
  ...
  test('terminal shows system messages during execution', async ({ page }) => {
    ...
    const hasCompleteMsg = messages.some((m) => m.includes('Bundle complete') || m.includes('Executing'));
...
test.describe('File Management', () => {
  ...
  test('clicking a file in the explorer switches the active file', async ({ page }) => {
  ...
  test('creating a new file via the + button', async ({ page }) => {
    ...
    const fileList = page.getByRole('listbox', { name: 'File list' });
    await expect(fileList.getByRole('option', { name: /config\.json/ })).toBeVisible();
  ...
  test('creating a file without extension auto-appends .ts', async ({ page }) => {
    await page.getByTitle('New File').click();
    const input = page.getByPlaceholder('filename.ts');
    ...
    const fileList = page.getByRole('listbox', { name: 'File list' });
    await expect(fileList.getByRole('option', { name: /mymodule\.ts/ })).toBeVisible();
  });
  ...
  test('deleting a file removes it from the explorer', async ({ page }) => {
    ...
    const deleteUtilsBtn = page.getByRole('button', { name: 'Delete utils.ts' });
    await deleteUtilsBtn.click();

    // utils.ts should be gone
    const fileList = page.getByRole('listbox', { name: 'File list' });
    await expect(fileList.getByRole('option', { name: /utils\.ts/ })).not.toBeVisible();
  });

  test('delete buttons are hidden when only one file remains', async ({ page }) => {
...
test.describe('Terminal Interactions', () => {
  ...
  test('clear button is only visible when there are logs', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Clear terminal' })).not.toBeVisible();
  ...
  test('running code again replaces previous output', async ({ page }) => {
    ...
    expect(secondRunMessages.length).toBeLessThanOrEqual(firstRunMessages.length + 1);
  });
});
...
test.describe('Multi-File Execution', () => {
  ...
  test('code that imports from another file works end-to-end', async ({ page }) => {
  ...
  test('editing an imported file and re-running reflects the change', async ({ page }) => {
