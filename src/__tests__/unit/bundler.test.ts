...
describe("bundler — VFS file resolution algorithm", () => {
  ...
  describe("resolveRelativeImport", () => {
    ...
    it("resolves ./utils from root importer to utils.ts", () => {
      expect(resolveRelativeImport("./utils", "index.ts", files)).toBe(
        "utils.ts",
      );
    });
    ...
    it('resolves bare path "math" from helpers/ importer to helpers/math.ts', () => {
      ...
      expect(resolveRelativeImport("math", "helpers/index.ts", files)).toBe(
        "helpers/math.ts",
      );
    });

    it("resolves ./helpers to helpers/index.ts (index file convention)", () => {
      expect(resolveRelativeImport("./helpers", "index.ts", files)).toBe(
        "helpers/index.ts",
      );
    });

    it("resolves ./data to data.json", () => {
      expect(resolveRelativeImport("./data", "index.ts", files)).toBe(
        "data.json",
      );
    });

    it("resolves ./component to component.tsx", () => {
      expect(resolveRelativeImport("./component", "index.ts", files)).toBe(
        "component.tsx",
      );
    });

    it("resolves ./legacy to legacy.js", () => {
      expect(resolveRelativeImport("./legacy", "index.ts", files)).toBe(
        "legacy.js",
      );
    });

    it("resolves ./react-comp to react-comp.jsx", () => {
      expect(resolveRelativeImport("./react-comp", "index.ts", files)).toBe(
        "react-comp.jsx",
      );
    });
    ...
    it("respects extension priority: .ts wins over .tsx, .js, .jsx", () => {
      ...
      expect(
        resolveRelativeImport("./widget", "index.ts", ambiguousFiles),
      ).toBe("widget.ts");
    });
    ...
    it("falls through to .tsx when .ts does not exist", () => {
      ...
      expect(resolveRelativeImport("./widget", "index.ts", tsxFiles)).toBe(
        "widget.tsx",
      );
    });
    ...
    it("resolves exact path (with extension) directly", () => {
      ...
      expect(
        resolveRelativeImport("./config.json", "index.ts", exactFiles),
      ).toBe("config.json");
    });
  });
